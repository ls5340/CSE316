...
export default class Top5Model {
    ...
    changeListName(id, newName) {
        if (newName) {
            if (this.top5Lists[this.getListIndex(id)].getName() !== newName)
                this.top5Lists[this.getListIndex(id)].setName(newName);
        }
        this.sortLists();
        this.view.refreshLists(this.top5Lists);
    }
    ...
    checkCurrentList(id) {
        if (this.hasCurrentList && this.currentList.id == id)
            return true;
        return false;
    }
    ...
    unselectAll() {
        for (let i = 0; i < this.top5Lists.length; i++) {
            this.view.unhighlightList(this.top5Lists[i].id);
        }
    }
    ...
    moveItem(from, to) {
        let text = this.currentList.items[from - 1];
        if (from > to) {
            for (let i = from; i > to; i--) {
                this.currentList.items[i - 1] = this.currentList.items[i - 2];
            }
        }
        else if (from < to) {
            for (let i = from; i < to; i++) {
        ...
        this.currentList.items[to - 1] = text;
        this.view.update(this.currentList);
        this.saveLists();
    }
